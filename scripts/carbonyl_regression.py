...
# -*- coding: utf-8 -*-
"""
Carbonyl Regression Analysis
----------------------------
Fits a robust linear model for carbonyl (C) chemical shifts using:
    y = ΔδC_i
    x = ΔδC_i − ΔδC_{i+1}
...
1) NMR-STAR (.str/.star) via pynmrstar (if installed)
   ...
   - Optionally give a random-coil (RC) CSV via --rc-table to compute ΔδC.
     The CSV must have headers: resname, RC_C  (resname = 3-letter code).

2) CSV mode
   - Use --input-format csv
   - CSV must contain at least:
       entity_assembly_id, entity_id, comp_index_id, comp_id, C
     Optionally include RC_C; if present, ΔδC = C - RC_C; otherwise ΔδC = C.
   - You can rename columns via CLI flags if your CSV headers differ.
...
OUTPUTS
-------
- Prints slope, intercept (offset), and simple diagnostics.
...
class CarbonylDatum:
    key: ResidueKey
    C: float                 # observed carbonyl chemical shift
    rc_C: Optional[float]    # random-coil reference (if known)

    @property
    def delta_C(self) -> float:
        """ΔδC = observed - RC (if RC present), else treat observed as Δδ already."""
        if self.rc_C is not None:
            return float(self.C) - float(self.rc_C)
        return float(self.C)


def load_rc_table(rc_csv: Path) -> Dict[str, float]:
    """
    Load random-coil table CSV: expects columns 'resname','RC_C'
    Returns dict mapping 3-letter residue name (upper) -> RC_C float
    """
    rc: Dict[str, float] = {}
    with rc_csv.open("r", newline="") as f:
        for row in csv.DictReader(f):
            name = str(row.get("resname", "")).strip().upper()
            val = row.get("RC_C", None)
...
def load_from_star(star_path: Path, rc_table: Optional[Dict[str, float]] = None) -> List[CarbonylDatum]:
    """Robust STAR reader for Atom_chem_shift loops.
    - Accepts tag names with/without category prefixes (e.g., _Atom_chem_shift.Atom_ID).
    - Tolerates different pynmrstar Loop APIs (tags, get_tag_names, tag_names).
    """
    ...
    def get_loop_tags(loop):
        ...
        # Fallback: infer from loop.tag_order if present
    ...
    for loop in loops:
        ...
        for row in loop.data:
            ...
            try:
                cval = float(str(row[name_to_idx[col_val]]))
            except Exception:
                continue
            ...
            if rc_table is not None:
                rcC = rc_table.get(comp.upper())

            key = ResidueKey(assemb, ent, comp_i, comp)
            records.append(CarbonylDatum(key=key, C=cval, rc_C=rcC))

    if not records:
        raise ValueError("No carbonyl (C) shifts found in the STAR file (category Atom_chem_shift).")
    return records


def load_from_csv(csv_path: Path,
                  col_map: Dict[str, str],
                  rc_in_csv: bool = True) -> List[CarbonylDatum]:
    """
    CSV must contain columns per col_map:
      entity_assembly_id, entity_id, comp_index_id, comp_id, C
    Optionally RC_C if rc_in_csv=True.
    """
    req = ["entity_assembly_id", "entity_id", "comp_index_id", "comp_id", "C"]
    for k in req:
        if k not in col_map:
            raise ValueError(f"Missing column mapping for '{k}'.")

    out: List[CarbonylDatum] = []
    with csv_path.open("r", newline="") as f:
        ...
        for row in reader:
            try:
                ...
                cval   = float(str(row[col_map["C"]]).strip())
            except Exception:
                continue

            rcC = None
            if rc_in_csv and "RC_C" in col_map:
                rc_str = row.get(col_map["RC_C"], "")
                if rc_str not in (None, ""):
                    try:
                        rcC = float(str(rc_str))
                    except ValueError:
                        rcC = None

            key = ResidueKey(assemb, ent, comp_i, comp)
            out.append(CarbonylDatum(key=key, C=cval, rc_C=rcC))
...
def build_neighbor_pairs(data: List[CarbonylDatum], rc_required: bool = False) -> Tuple[np.ndarray, np.ndarray, List[ResidueKey]]:
    """
    ...
    chains: Dict[Tuple[str, str], List[CarbonylDatum]] = defaultdict(list)
    for d in data:
    ...
    for chain_key, residues in chains.items():
        ...
        for k in range(len(residues_sorted) - 1):
            ...
            # If RC is required, skip if either residue lacks rc_C
            if rc_required and (i.rc_C is None or j.rc_C is None):
                continue
            di = i.delta_C
            dj = j.delta_C
            X.append(di - dj)
    ...
    if len(X) == 0:
        raise ValueError("No consecutive (i, i+1) pairs found for carbonyls.")
...
def fit_model(X: np.ndarray, y: np.ndarray, method: str = "huber") -> FitResult:
    ...
    if method == "ols":
        ...
        reg.fit(X, y)
        yhat = reg.predict(X)
        # pseudo-R2 (against OLS baseline)
...
def save_pairs_csv(path: Path, X: np.ndarray, y: np.ndarray, tags: List[ResidueKey],
    ...
    with path.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["entity_assembly_id", "entity_id", "comp_index_id", "comp_id", "x(ΔδC_i-ΔδC_{i+1})", "y(ΔδC_i)", "inlier"])
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Carbonyl robust regression: y=ΔδC_i vs x=ΔδC_i−ΔδC_{i+1}")
    ...
    p.add_argument("--rc-table", type=str, default=None,
                   help="CSV with columns resname,RC_C (used for ΔδC = C - RC_C).")
    ...
    p.add_argument("--rc-policy", choices=["optional", "require", "none"], default="optional",
                   help="How to handle random-coil references: optional (default), require (error/drop if missing), or none (treat observed C as ΔδC).")
    ...
    p.add_argument("--rc-from", choices=["module", "csv"], default="module",
                   help="Where to fetch random-coil values from. 'module': use uploaded random_coil.py; 'csv': use RC_C column / --rc-table averages.")
    ...
    p.add_argument("--outdir", type=str, default="carbonyl_out")
    ...
    p.add_argument("--csv-col-C", default="C")
    p.add_argument("--csv-col-RC_C", default="RC_C",
                   help="If present in CSV and you want to use it, keep name or map accordingly.")
    p.add_argument("--no-rc-in-csv", action="store_true",
                   help="If set, ignore RC_C column in CSV even if present.")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    # Load data
    if fmt == "star":
        data = load_from_star(in_path, rc_table=rc_table)
    elif fmt == "csv":
        col_map = {
        ...
        # Only include RC_C mapping if we intend to use it
        if not args.no_rc_in_csv and args.csv_col_RC_C:
            col_map["RC_C"] = args.csv_col_RC_C
            rc_in_csv = True
        else:
            rc_in_csv = False
        data = load_from_csv(in_path, col_map=col_map, rc_in_csv=rc_in_csv)
        # If RC_C missing in CSV but RC table provided, enrich
        if rc_table is not None:
            for d in data:
                if d.rc_C is None:
                    d.rc_C = rc_table.get(d.key.comp_id.upper())
    else:
    ...
    if args.rc_from == "module" and args.rc_policy != "none":
        ...
        for d in data:
            try:
                d.rc_C = rc_obj.get_value(d.key.comp_id, 'C', rc_models)
            except Exception:
                # leave None; policy will handle it
                pass
    ...
    if args.rc_policy == "none":
        # Zero-out RC so that delta_C == observed C
        for d in data:
            d.rc_C = None
    else:
        # If RC table provided, backfill missing RCs
        if rc_table is not None:
            for d in data:
                if d.rc_C is None:
                    d.rc_C = rc_table.get(d.key.comp_id.upper())
        if rc_required:
            # If any residue lacks RC, either error or allow dropping pairs later
            missing = sum(1 for d in data if d.rc_C is None)
            if missing > 0 and args.on_missing_rc == "error":
                raise SystemExit(f"Random-coil values missing for {missing} residues while --rc-policy=require. "
                                 f"Provide --rc-table or CSV RC_C, or use --on-missing-rc=drop to drop such pairs.")
    ...
    # Report
    print("\nCarbonyl Robust Regression (split sides)")
    print("--------------------------------")
